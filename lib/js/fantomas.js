...
function Fantomas(params) {
	...
	this.client.page.urlHitLimit = (h === undefined ? -1 : h);
...
Fantomas.prototype = {
	...
	open: function(url, openCallback) {
		...
		this.client.open(url, cb, this.headers);
	},
	...
	smartClicks: function(onDone) {
		...
		var rc = fantomas.interactor.randomClicks(fantomas.params.clicks);
		fantomas.waitAndDo(function() {
			debug(" -+- clicks simulated (hardware) : " + JSON.stringify(rc));
			fantomas.reportLinks();
			
			rc = fantomas.interactor.fireUXListeners(fantomas.client, false);
			fantomas.waitAndDo(function() {
